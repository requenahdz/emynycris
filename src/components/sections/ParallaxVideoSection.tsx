...
export default function ParallaxVideoSection() {
  ...
  return (
    <section ref={sectionRef} className="relative w-full overflow-hidden bg-[#e9e1d5] aspect-video md:aspect-auto md:min-h-screen">
      <div
        className="hidden md:block absolute inset-0"
        style={{ backgroundImage: "url(/assets/images/fondo-2.png)", backgroundRepeat: "repeat", backgroundSize: "720px auto" }}
      />

      <div ref={videoRef} className="absolute inset-0 md:inset-[-25%] z-10 will-change-transform">
        <video
          className="w-full h-full object-contain md:object-cover [filter:grayscale(100%)_sepia(45%)_brightness(110%)_contrast(86%)]"
          poster="/assets/images/fondo-2.png"
          autoPlay
          muted
          loop
          playsInline
